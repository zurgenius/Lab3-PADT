...
public:
  ...
  virtual Sequence<T> *slice(int index, int count,
                             const Sequence<T> *replace_seq = nullptr);
...
public:
  ...
  MutableArraySequence(const DynamicArray<T> &other)
...
public:
  ...
  ImmutableArraySequence(const T *items, int count)
      : ArraySequence<T>(items, count) {}
  ImmutableArraySequence(const DynamicArray<T> &other)
      : ArraySequence<T>(other) {}
