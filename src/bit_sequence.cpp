...
bool BitSequence::get_bit(int index) const {
  const unsigned char byte = data.get(index / 8);
...
Option<Bit> BitSequence::try_get_first() const {
  return (bit_count == 0) ? Option<Bit>::None()
                          : Option<Bit>::Some(Bit(get_bit(0)));
}

Option<Bit> BitSequence::try_get_last() const {
  return (bit_count == 0) ? Option<Bit>::None()
                          : Option<Bit>::Some(Bit(get_bit(bit_count - 1)));
}

Option<Bit> BitSequence::try_get(int index) const {
  return (index < 0 || index >= bit_count)
             ? Option<Bit>::None()
...
Sequence<Bit> *BitSequence::get_sub_sequence(int start, int end) {
  if (start < 0 || end < 0 || start >= bit_count || end >= bit_count ||
      start > end) {
    throw std::out_of_range("Index out of range");
  }
  ...
  result->data = DynamicArray<unsigned char>(check_bytes_needed(new_count));
...
Sequence<Bit> *BitSequence::append(const Bit &item) {
  const int new_count = bit_count + 1;
  const int new_bytes = check_bytes_needed(new_count);

  if (new_bytes != data.get_size()) {
    DynamicArray<unsigned char> rebuilt(new_bytes);
    zero_bytes(rebuilt);
    for (int index = 0; index < bit_count; index++) {
      if (get_bit(index)) {
        const int byte_index = index / 8;
        const int bit_index = index % 8;
        rebuilt.set(byte_index,
                    static_cast<unsigned char>(rebuilt.get(byte_index) |
                                               (1u << bit_index)));
      }
    }
    data = rebuilt;
...
Sequence<Bit> *BitSequence::prepend(const Bit &item) {
  const int new_count = bit_count + 1;
  BitSequence rebuilt;
  rebuilt.data = DynamicArray<unsigned char>(check_bytes_needed(new_count));
  rebuilt.bit_count = new_count;
  zero_bytes(rebuilt.data);

  rebuilt.set_bit(0, item.get());
  for (int index = 0; index < bit_count; index++) {
    rebuilt.set_bit(index + 1, get_bit(index));
  }

  data = rebuilt.data;
  bit_count = rebuilt.bit_count;
  return this;
}
...
Sequence<Bit> *BitSequence::insert_at(const Bit &item, int index) {
  ...
  for (int current = 0; current < index; current++) {
    rebuilt.set_bit(current, get_bit(current));
  }
  rebuilt.set_bit(index, item.get());
  for (int current = index; current < bit_count; current++) {
    rebuilt.set_bit(current + 1, get_bit(current));
  }
...
Sequence<Bit> *BitSequence::concat(const Sequence<Bit> *other) {
  ...
  const int new_count = bit_count + other->get_count();
  BitSequence rebuilt;
  rebuilt.data = DynamicArray<unsigned char>(check_bytes_needed(new_count));
  rebuilt.bit_count = new_count;
  zero_bytes(rebuilt.data);
  ...
  for (int index = 0; index < original_count; index++) {
    rebuilt.set_bit(index, get_bit(index));
  }
  for (int index = 0; index < other->get_count(); index++) {
    rebuilt.set_bit(original_count + index, other->get(index).get());
  }
...
Sequence<Bit> *BitSequence::map(Bit (*func)(const Bit &elem)) {
  BitSequence *result = new BitSequence();
  result->data = DynamicArray<unsigned char>(data.get_size());
...
Bit BitSequence::reduce(Bit (*func)(const Bit &first_elem,
                                    const Bit &second_elem),
...
BitSequence *BitSequence::bit_and(const BitSequence &other) const {
  const int result_count =
      (bit_count < other.bit_count) ? bit_count : other.bit_count;
  BitSequence *result = new BitSequence();
  result->data = DynamicArray<unsigned char>(check_bytes_needed(result_count));
