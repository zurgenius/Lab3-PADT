...
void menu_create_sequence() {
  ...
  if (choice == 5) {
    ...
    std::cout << "Stored as bit sequence " << (bit_sequence_count - 1)
              << std::endl;
    return;
  }
...
void menu_get_element() {
  ...
  try {
    std::cout << "Element: " << (*sequences[sequence_index])[item_index]
              << std::endl;
...
void menu_apply_mask() {
  ...
  try {
    Sequence<int> *result =
        bit_sequences[mask_index]->apply_mask(sequences[sequence_index]);
    std::cout << "Result: ";
    print_sequence(result);
